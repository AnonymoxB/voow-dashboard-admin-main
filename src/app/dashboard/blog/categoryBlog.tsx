...
import {
  deleteCategoryBlogService,
  getAllCategoryBlogService,
  showCategoryBlogService,
} from "@/services/categoryBlogService";
import {
  CategoryBlog,
  ResponseCategoryBlog,
  ResponseCategoryBlogShow,
} from "@/types/categoryBlog";
import { getToken } from "@/utils/auth";
import React, { useCallback, useEffect, useMemo, useState } from "react";
import { createColumnHelper } from "@tanstack/react-table";
import AddCategoryBlog from "./addCategoryBlog";
...
export default function CategoryBlog() {
  ...
  const [categoryBlogDataShow, setCategoryBlogDataShow] = useState({});
  const [showModal, setShowModal] = useState(false);

  async function getAllCategoryBlog() {
  ...
  async function deleteCategoryBlog(id: number) {
    ...
    Swal.fire({
      ...
      preConfirm: async () => {
        const categoryBlogDelete = await deleteCategoryBlogService(id, token);
        return categoryBlogDelete;
      },
  ...
  async function refreshData() {
    const data:any = await getAllCategoryBlog();
    setCategoryBlog(data.data.items);
  }

  useEffect(() => {
    async function dataAllCategoryBlog() {
      const data:any = await getAllCategoryBlog();
      setCategoryBlog(data.data.items);
    }
    dataAllCategoryBlog();
  ...
  const columnHelper = createColumnHelper<CategoryBlog>();

  const categoryBlogColumnDefs = useMemo<ColumnDef<CategoryBlog>[]>(
    () => [
  ...
  const showEditData = useCallback(
    async (slug: string) => {
      ...
      const token: string = getToken() ?? "";
      setShowModal(true);
      const result: any = await showCategoryBlogService(slug, token);

      const { data }: { data: ResponseCategoryBlogShow } = result;
      setCategoryBlogDataShow(data.data.item);

      const modal: any = document.getElementById("modalEditCategoryBlog");
      if (modal !== null) {
        modal.showModal();
      } else {
        console.error("Element with ID 'modalAddCategoryBlog' not found");
      }
    },
    [categoryBlogDataShow]
  );
    ...
    <>
      ...
      <section className="">
        <div className="flex text-gray-500 text-base pt-8">
          {!isPending ? (
            <Table data={categoryBlog} columns={categoryBlogColumnDefs}></Table>
          ) : (
      ...
      {/* modal add */}
      <AddCategoryBlog refreshData={refreshData}></AddCategoryBlog>

      {showModal ? (
        <EditCategoryBlog
          refreshData={refreshData}
          data={categoryBlogDataShow}
        ></EditCategoryBlog>
      ) : null}
    </>
  );
