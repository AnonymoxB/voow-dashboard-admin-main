...
import { deleteBlogService, getAllBlogService } from "@/services/blogService";
import { Blog, ResponseBlog } from "@/types/blog";
...
export default function Blog() {
  ...
  const blogColumnDefs = useMemo<ColumnDef<Blog>[]>(
    () => [
      ...
      columnHelper.accessor((row: any) => row.blog_category.title, {
        id: "category",
  ...
  async function getBlogData() {
    ...
    const result: any = await getAllBlogService(token);
    const { data }: { data: ResponseBlog } = result;
  ...
  async function deleteTutorial(id: number) {
    ...
    Swal.fire({
      ...
      preConfirm: async () => {
        const result: any = await deleteBlogService(id, token);
        return result;
      },
  ...
  async function refreshData() {
    const blog: any = await getBlogData();
    setBlogData(blog);
  }
    ...
    <>
      ...
      <section className="">
        <div className="flex text-gray-500 text-base pt-8">
          {!isPending ? (
            <Table data={blogData} columns={blogColumnDefs}></Table>
          ) : (
